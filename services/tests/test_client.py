...
class BaseKinopoiskTestCase(unittest.TestCase):
    ...
    def test_multiple_info_generates_expected_requests(self, mock_load_json):
        ...
        for test_input in inputs:
            ...
            params = {
                url_query_names[param]: str(value).lower() if param != 'title_ids' else value
                for param, value in test_input.items()
            }
            urls.append(base_url + urlencode(params, doseq=True))
...
class KinopoiskKeywordsTestCase(unittest.TestCase):
    ...
    def test_load_keywords_builds_correct_url_and_returns_data(self, mock_load_json):
        ...
        urls = [base_url + urlencode(case, doseq=True) for case in test_cases]
        ...
        for i, case in enumerate(test_cases):
            with self.subTest(params=case):
                ...
                self.assertEqual(
                    self.success_output['docs'],
                    client._load_keywords(title_ids=title_ids if isinstance(title_ids, list) else None),
                )
...
class KinopoiskImagesTestCase(unittest.TestCase):
    ...
    def test_get_multiple_backdrops_paginates_across_multiple_pages(self, mock_load_images):
        ...
        json_output = [
            {'docs': docs[i : i + backdrops_per_title], 'total': titles_count * backdrops_per_title}
            for i in range(0, titles_count * backdrops_per_title + 1, backdrops_per_title)
        ]
