...
from services.kinopoisk_api import TitleWrapper, KinopoiskClient
from titles.models import Title, Statistic, Poster, Backdrop, Group, Studio, Person, SeasonsInfo
...
def data_initialization(configuration):
    ...
    titles = instance.get_multiple_info(limit=configuration['limit'], page=configuration['page'], rating=configuration['rating'],
                                 is_series=configuration['is_series'], year=configuration['year'], genre=configuration['genre'])
    ...
    if seqs_and_preqs_permission and nonexistent_seqs_and_preqs:
        ...
        for i in range(0, len(nonexistent_seqs_and_preqs) + 1, step):
            seqs_and_preq_creation_candidates += instance.get_multiple_info(title_ids=list(nonexistent_seqs_and_preqs)[i:i+step])
...
def create_movie_objs(data_to_create, title_ids):
    if data_to_create:
        ...
        for obj in data_to_create:
            objs[obj.title_id] = Title(kinopoisk_id=obj.title_id, name=obj.name, alternative_name=obj.alternative_name,
                                 status=obj.status, overview=obj.overview, tagline=obj.tagline, age_rating=obj.age_rating,
                                 premiere=obj.premiere, year=obj.year, names=obj.names, imdb_id=obj.imdb_id, tmdb_id=obj.tmdb_id,
            ...
            statistic = Statistic(kp_rating=obj.ratings['kp'], kp_votes=obj.votes['kp'],
                                imdb_rating=obj.ratings['imdb'], imdb_votes=obj.votes['imdb'],
                                title=objs[obj.title_id])
            statistics.append(statistic)

            seasons_info = obj.seasons_info
            episodes += generate_episode_objs(seasons_info, cur_title) if seasons_info else [SeasonsInfo(title=cur_title)]

            if obj.poster:
                poster = cur_title.upload_poster(obj.poster)
                if poster is not None: posters.append(poster)
        ...
        if any(objs.values()):
            ...
            if statistics: Statistic.objects.bulk_create(statistics)
            if posters: Poster.objects.bulk_create(posters)
            if episodes: SeasonsInfo.objects.bulk_create(episodes)
    ...
    if any(data_to_join.values()):
        ...
        for title_id, studios in data_to_join.items():
            ...
            for name in studios:
                ...
                if studio and title: rels.append(related_model(title=title, studio=studio))
        if rels:
...
def join_persons(created_objs, data_to_join):
    if any(data_to_join.values()):
        ...
        persons_to_create = {person['id']: Person(kinopoisk_id=person['id'], name=person['name'], description=person['description'],
                            profession=person['enProfession'], image=person['photo']) for person in incoming_persons}
        ...
        for i in range(0, len(persons_to_create), batch_size):
            ...
            person_objs.update({person.kinopoisk_id: person for person in Person.objects.filter(kinopoisk_id__in=batch)})
        ...
        for title_id, persons in data_to_join.items():
            ...
            for person in persons:
                ...
                if person_obj and title: rels.append(related_model(title=title, person=person_obj))
        if rels:
...
def join_genres(created_objs, data_to_join):
    if any(data_to_join.values()):
        ...
        if missing_genres:
            genres_to_create = (
                Collection(name=name, type=Collection.GENRE, slug=unidecode(name).translate(name).replace(' ', '_').lower())
        ...
        for title_id, genres in data_to_join.items():
            ...
            for name in genres:
                ...
                if genre and title: rels.append(related_model(title=title, collection=genre))
        if rels:
            related_model.objects.bulk_create(rels, ignore_conflicts=True)
