...
function getChartTitles() {
    const url = window.INDEX.getChart
    ...
    chartButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            ajax_get(url, data).then(response => displayTitles(response.data, chart));
        });
    });
}

async function displayTitles(data, chartType) {
    const list = document.getElementById('chart');
    const userTitles = (await loadUserTitles()).items;

    list.innerHTML = '';

    const icon = chooseIcon(chartType);
    let html = ''
    ...
    for (const title of titles) {
        i += 1
        let chartValue = 0;
        const genres = title.genres?.length ? title.genres.join(' | ') : '—';
        if (title.views) {
            chartValue = humanizeNumber(title.views);
        } else if (title.rating) {
            chartValue = title.rating;
        } else if (title.comments) {
            chartValue = humanizeNumber(title.comments);
        }
        if (title.small_poster && title.medium_poster) {
            backdrop = `background: url(${title.small_poster}) no-repeat center left;`
            poster = title.medium_poster;
        }
        html +=
            `
            <li class="media-card absolute-center" style="${backdrop}">
                <a href="${title.url}" title='Смотреть онлайн "${title.name}"' class="w-20 h-full shrink-0">
                    <img class="w-full h-full rounded-[10px]" src="${poster}" alt="${title.name}">
                </a>
                <div class="relative h-full flex items-center justify-between w-full">
                    <div>
                        <div class="media-card__title flex items-center mb-2 gap-2">
                            <span class="flex flex-center font-bold w-8 h-8 bg-[rgba(31,41,55,0.5)] rounded-[10px]">${i}</span>
                            <h4 class="text-[1.2rem] line-clamp-2 text-ellipsis max-w-[300px] font-bold">
...
function chooseIcon(chartType) {
    ...
    if (chartType === charts.POPULAR) {
     ...
     if (iconInner) {
        icon =
            `
            <svg fill='white' width="25px" height="18px" viewBox="${viewBox}" xmlns="http://www.w3.org/2000/svg">
                ${iconInner}
            </svg>
            `
     }
