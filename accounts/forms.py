...
class UserLoginForm(AuthenticationForm):
    username = forms.CharField(widget=forms.TextInput(
        attrs={'class': 'input-field',
               'placeholder': 'Введите ваш логин или email'}), required=True)
    password = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'input-field',
               'placeholder': 'Введите ваш текущий пароль'}), required=True)
...
class UserRegisterForm(UserCreationForm):
    ...
    password2 = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'input-field',
               'placeholder': 'Введите пароль ещё раз'}), required=True)

    # We need to modify save here (no in the model), because we cannot manipulate user without commit
    def save(self, commit=True):
        if commit:
            user = super().save()
            verification_record = EmailVerification.objects.create(code=uuid.uuid4(), user=user,
...
class EmailForm(forms.Form):
    ...
    def clean_email(self):
        ...
        verification_record = EmailVerification.objects.create(code=uuid.uuid4(), user=user.first(),
                                                               expiration=now() + timedelta(hours=1),
