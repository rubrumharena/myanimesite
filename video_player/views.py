...
class VideoPlayerAjaxView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        episode, season, voiceover_id = request.GET.get('episode'), request.GET.get('season'), request.GET.get('voiceover')
        ...
        try:
            ...
            return JsonResponse(data=ViewingHistory.get_track_info(resource, title), status=HTTPStatus.OK)
        except TypeError:
            ...
        ...
        if user.is_authenticated:
            viewing_record = (ViewingHistory.objects
                              .filter(user=user, resource__content_unit__title=title)
                              .order_by('-watched_at')
                              .select_related('resource')
                              .first()) or ViewingHistory()
        ...
        if request_for_series or request_for_movie:
            resource = VideoResource.objects.resolve_resource(episode=episode, season=season, voiceover_id=voiceover_id, title_id=title.id)
            if not resource:
        ...
        if not content:
            viewing_record = ((ViewingHistory.objects
                                  .filter(user=user, resource=resource)
                                  .select_related('resource')
                                  .first()) or ViewingHistory()) if resource and user.is_authenticated else ViewingHistory()
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = request.POST
        episode, season, voiceover_id = data.get('episode'), data.get('season'), data.get('voiceover')
        ...
        resource = VideoResource.objects.resolve_resource(episode=episode, season=season, voiceover_id=voiceover_id, title_id=data.get('title_id'))
        if not resource:
        ...
        return_data = {'season': season, 'voiceover': voiceover_id, 'episode': episode, 'position': position}
