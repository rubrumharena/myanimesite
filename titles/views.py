...
from common.utils.wrappers import superuser_required, login_required_ajax
from common.utils.enums import ChartType
from common.utils.validators import check_single_rating_part
from common.views.mixins import PageTitleMixin, FolderFormMixin
from titles.documents import TitleDocument
from titles.forms import TitleForm
from titles.models import Title, Statistic, RatingHistory
from titles.models import TitleCreationHistory
from services.kinopoisk_import import data_initialization, create_movie_objs
    ...
    def get_context_data(self, **kwargs):
        ...
        selections = {
            'releases': base_q.only('id', 'name', 'poster', 'premiere', 'type', 'statistic'
                                                ).filter(premiere__lte=today).order_by('-premiere')[:20],
            'most_viewed_titles': base_q.only('id', 'name', 'year', 'type', 'poster', 'statistic'
                                        ).order_by('-statistic__views')[:10],
            'upcoming_titles': base_q.only('id', 'name', 'poster', 'premiere', 'type', 'statistic'
                                     ).filter(premiere__gt=today).order_by('-premiere')[:20]
        }
...
class TitleDetailView(PageTitleMixin, FolderFormMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.kwargs['type'] != types[self.object.type]:
            return HttpResponseRedirect(reverse('titles:title_page', kwargs={'type':types[self.object.type], 'title_id': self.object.pk}))
    ...
    def get_object(self, queryset = ...):
        title = (
            ...
            .with_filmmakers()
            .prefetch_related(
                Prefetch('collections', queryset=Collection.objects.filter(type=Collection.GENRE),
    ...
    def get_context_data(self, **kwargs):
        ...
        voiceovers = VideoResource.objects.filter(content_unit__title=title_obj).values_list('voiceover__name', flat=True).distinct()

        try:
            filled_star_rating = get_partial_fill(title_obj.statistic.rating)
            is_rated = RatingHistory.objects.filter(user=self.request.user, title=title_obj).exists() if self.request.user.is_authenticated else False
...
def search_ajax(request):
    ...
    if search_field:
        q = ES_Q('bool', should=[
            ...
            ES_Q('multi_match', query=search_field, fields=['name', 'alternative_name', 'names'], type='phrase_prefix')
            ])
...
def set_rating_ajax(request):
    ...
    obj, created = RatingHistory.objects.update_or_create(user=request.user, title_id=title_id, defaults={'rating': rating})
...
def get_chart_ajax(request):
    ...
    base_q = Title.objects.annotate(genres=ArrayAgg('collections__name',
                                        filter=Q(collections__type=Collection.GENRE),
                                        distinct=True)).only('id', 'name', 'type', 'year',
                                                             'poster', 'statistic').select_related('poster', 'statistic')
    match chart_type:
